...
mean_raw = os.path.join(processed_folder, 'full/January_MinimumTemperature_Raw_2km_2000-2015.tif')
mean_interpolated = os.path.join(processed_folder, 'sa/January_MinimumTemperature_Interpolated_2km_2000-2015.tif')
...
if arcpy.Exists(mean_raw) == 0:
    print('Calculating minimum January temperature...')
    arcpy_geoprocessing(calculate_climate_mean, **kwargs_mean)
    print('----------')
else:
    print('Minimum January temperature already exists.')
    print('----------')
...
for grid in grid_list:
    ...
    output_raster = os.path.join(output_path, 'January_MinimumTemperature_AKALB_' + grid + '.tif')
